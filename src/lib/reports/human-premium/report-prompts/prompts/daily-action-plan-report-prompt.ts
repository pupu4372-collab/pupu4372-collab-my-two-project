...
★ 문체 규칙 — 용어는 짧게, 설명은 현실적으로 (ko/en 공통):
  비견·겁재·식신·상관·정재·편재·정관·편관·정인·편인 같은 십신 용어나
  오행 한자는 "명사형으로 짧게 한 번" 언급하는 데서 그치고, 그 뒤에
  사전적 정의를 괄호로 설명하지 말 것. 대신 그 용어가 오늘 이
  사람에게 실제로 어떤 상황·감정·행동으로 나타나는지를 친구한테
  말하듯 편한 구어체로 풀어서 설명할 것.
  나쁜 예 (설명이 학술적): "상관(생각과 말로 승부를 보는 힘)이
  강해져 표현력이 좋아집니다."
  좋은 예 (설명이 현실적·구어체): "상관 기운이 세요. 그래서 오늘은
  하고 싶은 말이 술술 나오는 날이에요. 근데 그 말이 세게 나갈 수
  있으니까, 중요한 얘기는 한 번 더 다듬고 꺼내는 게 좋아요."
  ★ 목표: 용어 자체를 이해시키는 게 아니라, "아 오늘 나한테 이런
  일이 생길 수 있으니 이렇게 하면 되겠구나"를 이해시키는 것.
  ★ en 출력 시: sajuStructure 본문 전체를 자연스러운 일상 영어로
  작성할 것. 로마자 표기 명리 용어(Sangwan, Jeongjae, Mok, Hwa 등)를
  나열하지 말고, 완전히 풀어 쓸 것.
  (예: "You've got a strong drive to speak your mind today —
  just soften it before an important conversation.")`,
...
cohortInsight.body: 동일 명식 구조를 가진 사람들의 장기 통계 패턴 2줄 (120자)
  - ★ 한자 표기 예외 적용 — 간지·오행·십신은 한글로만 (예: "임수 일간", "상관·재성")
  - 주어는 반드시 "[일간]에 [특정 오행/십신] 구조를 가진 사람들은" 형태로 명식 기준 서술
  - 시간 범위는 제한하지 않음 — 수십 년 단위 인생 패턴(예: 특정 연령대의 커리어 전환,
    재정 재정비 시기 등)을 통계처럼 서술해도 됨
  - 통계 수치(%) 1~2개 포함 권장
  - ★ 금지: "이 상품/루틴/리포트를 선택한 독자·구매자·사용자" 등 상품 구매 행위를
    주어로 하는 자기지시적 표현 절대 금지 — 반드시 명식 구조가 주어여야 함
  예시: "임수 일간에 해·자 수기운이 강하고 상관·재성이 발달한 구조를 가진 사람들은,
  통계적으로 40대 중후반~50대 초반에 재정 구조를 재정비하며 두 번째 커리어를
  설계하는 경우가 많습니다. 이들 중 약 68%는 '말·기술·전문성'을 활용한 일에서
