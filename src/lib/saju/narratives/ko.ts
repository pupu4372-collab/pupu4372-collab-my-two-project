...
> = {
  wood: {
    headline: (name, romanized, hangul, hanja) =>
      `${name} · ${romanized}(${hangul}, ${hanja}) 숲길을 여는 성장형 에너지`,
    story: (name, romanized, hangul, species) =>
      [
        `${name}는 ${romanized}(${hangul}) 기운이 앞장서는 ${species}예요. 오래된 만세력으로 보면 木은 봄의 문을 여는 기운이라, ${name} 안에는 “일단 맡아보고, 가보고, 친해져 보자”는 작은 탐험대장이 살고 있어요.`,
  ...
  fire: {
    headline: (name, romanized, hangul, hanja) =>
      `${name} · ${romanized}(${hangul}, ${hanja}) 작은 태양 에너자이저`,
    story: (name, romanized, hangul, species) =>
      [
        `${name}는 ${romanized}(${hangul}) 기운이 반짝이는 ${species}입니다. 火는 등불이자 축제의 기운이라, ${name}는 조용히 있어도 존재감이 슬쩍 새어 나와요. 방 안 어딘가에서 이미 무대 조명이 켜진 느낌이랄까요.`,
  ...
  earth: {
    headline: (name, romanized, hangul, hanja) =>
      `${name} · ${romanized}(${hangul}, ${hanja}) 집안을 지키는 포근 수호자`,
    story: (name, romanized, hangul, species) =>
      [
        `${name}는 ${romanized}(${hangul}) 기운이 든든한 ${species}예요. 土는 가운데를 잡아주는 산과 들판의 기운이라, ${name}에게는 “내 사람, 내 자리, 내 밥그릇”을 소중히 여기는 안정감이 있습니다.`,
  ...
  metal: {
    headline: (name, romanized, hangul, hanja) =>
      `${name} · ${romanized}(${hangul}, ${hanja}) 달빛 같은 시크 관찰자`,
    story: (name, romanized, hangul, species) =>
      [
        `${name}는 ${romanized}(${hangul}) 기운이 맑게 선 ${species}입니다. 金은 가을 달빛처럼 선명하고 정제된 기운이라, ${name}는 아무거나 대충 넘기지 않는 타입이에요. 새 택배, 새 신발, 새 표정까지 조용히 감정합니다.`,
  ...
  water: {
    headline: (name, romanized, hangul, hanja) =>
      `${name} · ${romanized}(${hangul}, ${hanja}) 달그림자 직감형 소울`,
    story: (name, romanized, hangul, species) =>
      [
        `${name}는 ${romanized}(${hangul}) 기운이 깊은 ${species}예요. 水는 밤하늘과 물결의 기운이라, ${name}는 말보다 분위기를 먼저 읽는 편입니다. 집사님이 한숨을 쉬면 이미 알고 있었다는 듯 슬쩍 가까이 오는 재주가 있어요.`,
...
export function narrativeKo(
  ...
  return {
    headline: tpl.headline(petName, meta.meaning, meta.hangul, meta.hanja),
    story: tpl.story(petName, meta.meaning, meta.hangul, speciesLabel),
    traits: tpl.traits,
  };
}
